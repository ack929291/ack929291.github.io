...
最后一个Transformer Block输出的是5个2048维向量，仍然记作`(5, 2048)`。模型先对每个向量分别做一次归一化，使它们以相对稳定的数值尺度进入输出层。归一化会调整向量的数值尺度，处理后仍然是5个2048维向量，向量的个数和维度保持不变。Qwen2.5-3B在这里使用的归一化方法叫作RMSNorm。
